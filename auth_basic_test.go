...
func TestSimpleAuthenticator(t *testing.T) {
	...
	// constructor avoids nils
	usr := NewBasicUser("id", "name", "email", "pass", "key", []string{})
	...
	usr2 := NewBasicUser("id2", "name", "email", "pass", "key", []string{})
	assert.False(auth.CheckAuthenticated(usr2))

	usr3 := NewBasicUser("id3", "name", "email", "pass", "key", []string{"admin"})
	usr3broken := NewBasicUser("id3", "name", "email", "pass", "yek", []string{"admin"})
