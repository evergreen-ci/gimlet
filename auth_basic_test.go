...
func TestSimpleAuthenticator(t *testing.T) {
	...
	usr2 := NewBasicUser(&BasicUserOpts{"id2", "name", "email", "key", []string{}, ""})
	assert.False(auth.CheckAuthenticated(usr2))

	usr3 := NewBasicUser(&BasicUserOpts{"id3", "name", "email", "key", []string{"admin"}, ""})
	usr3broken := NewBasicUser(&BasicUserOpts{"id3", "name", "email", "yek", []string{"admin"}, ""})
