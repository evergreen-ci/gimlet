...
func (s *LDAPSuite) TestGetUserByToken() {
	...
	u, needsReauth, err := s.um.GetUserByToken(ctx, "foo")
	s.Error(err)
	s.False(needsReauth)
	s.Nil(u)

	setCacheGetUserByToken(s.serviceUserUm, mockGetErr)
	u, needsReauth, err = s.serviceUserUm.GetUserByToken(ctx, "foo")
	s.Error(err)
	s.False(needsReauth)
	s.Nil(u)
	...
	u, needsReauth, err = s.badGroupUm.GetUserByToken(ctx, "foo")
	s.NoError(err)
	s.False(needsReauth)
	s.Equal("foo", u.Username())
	...
	setCacheGetUserByToken(s.um, mockGetExpired)
	u, needsReauth, err = s.um.GetUserByToken(ctx, "badUser")
	s.Error(err)
	s.False(needsReauth)
	s.Nil(u)
	...
	u, needsReauth, err = s.realConnUm.GetUserByToken(ctx, "foo")
	s.Error(err)
	s.False(needsReauth)
	s.Nil(u)
}
